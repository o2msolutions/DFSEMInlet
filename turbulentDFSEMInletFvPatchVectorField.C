...
)
{
    ...
    if (global)
    {
        ...
        if (Pstream::master())
        {
            areaFraction = rndGen_.scalar01()*patchArea_;
        }
    ...
    else
    {
        ...
        scalar areaFraction = rndGen_.scalar01()*maxAreaLimit;
        ...
        while (search && iter++ < seedIterMax_)
        {
            ...
            if (faceI != -1)
            {
                ...
                (
                    faceI,
                    pos.hitPoint(),
                    -maxSigmaX_ + rndGen_.scalar01()*maxSigmaX_,
                    sigmax_[faceI],
                    R_[faceI],
                    rndGen_
                );
...
void Foam::turbulentDFSEMInletFvPatchVectorField::write(Ostream& os) const
{
    ...
    writeEntryIfDifferent<bool>(os, "writeEddies", false, writeEddies_);
